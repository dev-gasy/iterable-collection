...
class PartyEntity extends Entity<PartyData, QuoteData> {
  ...
  getTotalVehicleValue(): number {
    return this.getVehicles().reduce((total, vehicle) => total + vehicle.getValue(), 0);
  }
...
class QuoteEntity extends Entity<QuoteData, BusinessEntity> {
  ...
  getTotalVehicleValue(): number {
    return this.getParties().reduce((total, party) => total + party.getTotalVehicleValue(), 0);
  }
...
// Collection Classes
class VehicleCollection extends IterableCollection<VehicleData, VehicleEntity, PartyData> {
  ...
  getClassicVehicles(): VehicleCollection {
    const classicVehicles = this.toArray().filter(vehicle => vehicle.isClassic());
    const classicData = classicVehicles.map(vehicle => vehicle.raw()).filter((data): data is VehicleData => data != null);
    return new VehicleCollection(classicData, this.parent);
  }

  getTotalValue(): number {
    return this.toArray().reduce((total, vehicle) => total + vehicle.getValue(), 0);
  }
}

class DriverCollection extends IterableCollection<DriverData, DriverEntity, PartyData> {
  ...
  getHighRiskDrivers(): DriverCollection {
    const highRiskDrivers = this.toArray().filter(driver => driver.isHighRisk());
    const highRiskData = highRiskDrivers.map(driver => driver.raw()).filter((data): data is DriverData => data != null);
...
class QuoteCollection extends IterableCollection<QuoteData, QuoteEntity, BusinessEntity> {
  ...
  getActiveQuotes(): QuoteCollection {
    ...
    const activeData = activeQuotes.map(quote => quote.raw()).filter((data): data is QuoteData => data != null);
    return new QuoteCollection(activeData);
  }

  getHighRiskQuotes(): QuoteCollection {
    const highRiskQuotes = this.toArray().filter(quote => quote.hasHighRiskElements());
    const highRiskData = highRiskQuotes.map(quote => quote.raw()).filter((data): data is QuoteData => data != null);
    return new QuoteCollection(highRiskData);
  }

  getTotalPremium(): number {
    return this.toArray().reduce((total, quote) => total + quote.getPremium(), 0);
  }
}
...
describe("Integration: Fluent + IterableCollection + Entity", () => {
  ...
  describe("Lens-based navigation with complex data", () => {
    ...
    it("should calculate risk scores using lens navigation", () => {
      ...
      const hasYoungDrivers = drivers.some((d: DriverData) => new DriverEntity(d).getAge() < 25);

      const violationCounts = drivers.map((driver: DriverData) => driver.violations?.length ?? 0);
      const violationPoints = violationCounts.reduce((sum: number, count: number) => sum + count, 0);
    ...
    it("should handle missing data gracefully with lens navigation", () => {
      ...
      const totalVehicles = Array.isArray(parties) 
        ? parties.reduce((sum: number, party: any) => sum + (party?.vehicles?.length ?? 0), 0)
        : 0;
  ...
  describe("Combined usage patterns", () => {
    ...
    it("should handle data transformation pipeline", () => {
      ...
      const quoteSummaries = quotes.toArray().map(quote => {
        const raw = quote.raw();
        if (!raw) return null;
        
        // Use improved lens navigation for complex data extraction
        const parties = from(raw).safeProp("parties").getOr([]) as PartyData[];
        
        const vehicleCount = parties.reduce((sum: number, party: PartyData) => sum + (party?.vehicles?.length ?? 0), 0);
        const driverCount = parties.reduce((sum: number, party: PartyData) => sum + (party?.drivers?.length ?? 0), 0);
