...
describe("Collection update method", () => {
  it("should update collection items immutably", () => {
    ...
    const updatedCollection = collection.update((draft) => {
      // Update entities using their update method
      draft[0] = draft[0].update((entityDraft) => {
        entityDraft.name = "Updated First";
      });
      draft[1] = draft[1].update((entityDraft) => {
        entityDraft.value = 200;
      });
    });
  ...
  it("should handle complex bulk updates", () => {
    ...
    // Act
    const updatedCollection = collection.update((draft) => {
      // Update all values to be double using entity update method
      for (let i = 0; i < draft.length; i++) {
        draft[i] = draft[i].update((entityDraft) => {
          entityDraft.value = entityDraft.value * 2;
        });
      }

      // Add a new item
      const newEntity = new TestEntity(
        partial<TestData>({ _key: { id: "4" }, name: "Task 4", value: 80 })
      );
      draft.push(newEntity);
  ...
  it("should work with empty collection", () => {
    ...
    const updatedCollection = collection.update((draft) => {
      const newEntity = new TestEntity(
        partial<TestData>({ _key: { id: "1" }, name: "New Item", value: 100 })
      );
      draft.push(newEntity);
    });
